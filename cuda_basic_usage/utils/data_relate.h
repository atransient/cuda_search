...
static void
initImage(__nv_bfloat16* image, int64_t imageSize) {
    static unsigned seed = 123456789;
    for (int64_t index = 0; index < imageSize; index++) {
        seed         = (1103515245 * seed + 12345) & 0xffffffff;
        image[index] = __float2bfloat16(float(seed) * 2.3283064e-10f);  // 2^-32
    }
}
...
struct Surface {
   ...
   public:
    explicit Surface(int64_t n_elems, bool zero_init) : n_elems(n_elems) {
    ...
    void d2h() {
        CUDA_CHECK(cudaMemcpy(hostPtr, devPtr, size_t(sizeof(hostPtr[0]) * n_elems), cudaMemcpyDeviceToHost));
        CUDA_CHECK(cudaDeviceSynchronize());
    }
